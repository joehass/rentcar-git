            ...
            <div class="row">
                <div class="navbar-collapse collapse" >
                    <ul class="nav navbar-nav" style="border: 1px solid #ccc;border-radius:15px;">
                        ...
                        <li><a style="font-family: 仿宋;color:block;font-size: 2em;" href="#">联系客服</a></li>
                    	<li><a style="font-family: 仿宋;color:black;font-size: 2em;" href="<%=request.getContextPath()%>/car/getImg?u_card=${user.u_card}">车图一览</a></li>
                   		<li class="active"><a style="font-family: 仿宋;color:red;font-size: 2em;" href="<%=request.getContextPath()%>/order/baoxiu2?u_card=${user.u_card}">车辆报修</a></li>
                    </ul>
                </div>  
            </div>
        </div>
...
function yzf(){
	var o_state = "已支付";
	window.location.href='${pageContext.request.contextPath}/order/grtBaoxiuByTj?o_state='+o_state+'&u_card=${user.u_card}';
}
function wzf(){
	var o_state = "未支付";
	window.location.href='${pageContext.request.contextPath}/order/grtBaoxiuByTj?o_state='+o_state+'&u_card=${user.u_card}';
	...
	window.location.href='${pageContext.request.contextPath}/order/grtBaoxiuByTj?o_state='+o_state+'&u_card=${user.u_card}';
								...
								<table cellspacing="0" class="shop_table cart" id="table1">
                                	<tr class="cart_item">
                                        ...
                                        <td class="product-thumbnail">报修操作</td>
                                    </tr>
                                    <c:forEach var="o" items="${orders}">
                                    	<tr class="cart_item" id="ttr">
	                                        ...
	                                        <c:if test="${'已支付'==o.o_state }">
	                                        	<a href="#" onclick="dydd('${o.o_code }','${o.b_code}','${user.u_name}')" style="text-decoration:none;border-radius:5px; border:1px solid #99CCFF;background-color:#FFFFCC; font-family:楷体;font-size:1.3em;color:green;font-weight: 900;">保修</a>
	                                        </c:if>
	                                        </td>
                                   	    </tr>
                                    </c:forEach>
...
function dydd(o_code,b_code,u_name){	
	...
	var o_code1 = o_code;
	window.location.href='${pageContext.request.contextPath }/order/baoxiu?o_code='+o_code+'&u_card='+u_card+'&b_code='+b_code;
}
