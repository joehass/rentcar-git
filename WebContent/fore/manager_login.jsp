...
<head>
<title>登录界面</title>
<script src='<%=request.getContextPath() %>/fore/js/jquery.js'></script>
<script src="<%=request.getContextPath() %>/fore/js/prefixfree.min.js"></script>
<link href='<%=request.getContextPath() %>/fore/css/login.css' rel='stylesheet' type='text/css'>
<style>
body, html {
  height: 100%;
}
...
<form id="lg-form" action="${pageContext.request.contextPath }/manager/managerLogin" onsubmit="return checkAll()" method="post" >
		<!-- 验证码 -->
		<input type="text" id="sessionCode" style="height: 0;width: 0;">
		<div class="biaodan" >
			<h1 align="center" style="font-family:楷体;font-size: 2.2em;">管理员登录</h1>
			<br>
			<div>
				<input type="text" name="m_name" id="username" placeholder="请输入用户名"/>
			</div>
			 <span id="Tusername" style="font-weight: 900 ;color:red;">.</span>
			<div>
				<input type="password" name="m_psw" id="password" placeholder="请输入密码" />
			</div>
			 <span id="Tpassword" style="font-weight: 900;color:red;">.</span>
			<div style="margin-top:-10px;">
				<input onclick="ft()" style="width: 100px;" type="text" name="verifyCode" id="verifyCode" placeholder="验证码" />
				<img title="换一张" style="position:relative;top:10px ; width: 93px;height: 32px;border: 1px solid #ccc;border-radius:10px;" id="img"
						src="${pageContext.request.contextPath}/VerifyCodeServlet">
			</div>
			 <span id="TverifyCode" style="font-weight: 900;color:red;">.</span>
			<div>
				<button type="submit" id="login" style="color:#000000;font-family:楷体;">&nbsp;登&nbsp;录&nbsp;</button>
				&nbsp;&nbsp;&nbsp;&nbsp;
				<button type="reset" id="register" style="color:#000000;font-family:楷体;">&nbsp;重&nbsp;置&nbsp;</button><!-- onclick="reg()" -->
				<br><br>
				<a style="color:blue;font-size: 0.85em;margin-left: 20px;text-decoration: none;" href="<%=request.getContextPath()%>/fore/frist.jsp">回首页？</a>
				<a style="color:blue;font-size: 0.85em;margin-left: 40px;text-decoration: none;" href="<%=request.getContextPath() %>/user/registerJsp">新手注册？</a>
			</div>
		</div>
</form>
...
function ft(){
	$.post("${pageContext.request.contextPath}/sendVerifyCodeServlet?method=getsessionCode",function(data){
...
/* 跳转到登录界面 */
function reg(){
	window.location.href='<%=request.getContextPath() %>/user/registerJsp';
}
/* js校验 */
function checkAll(){
	return checkName()&&checkPew()&&checkCode()&&checkYCode();
}
function checkName(){
	var username = $("#username").val(); 
	if(username == ""){
		$("#Tusername").html('用户名不能为空！');
		return false;
	}else{
		$("#Tusername").html('.');
		return true;
