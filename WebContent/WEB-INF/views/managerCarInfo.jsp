            ...
            <div class="row">
                <div class="col-md-8">
                    ...
                    <div class="user-menu">
                        <ul>
                            <li>当前用户：</li>
                        	<li><a href="#"><i class="fa fa-user"></i>${manager.m_name}</a></li>
...
function cx(){
	...
	window.location.href='${pageContext.request.contextPath}/order/cxOrder?o_code='+o_code+'&u_card=${user.m_card}';
}
function yzf(){
	var o_state = "已支付";
	window.location.href='${pageContext.request.contextPath}/order/grtOrderByTj?o_state='+o_state+'&u_card=${user.m_card}';
}
function wzf(){
	var o_state = "未支付";
	window.location.href='${pageContext.request.contextPath}/order/grtOrderByTj?o_state='+o_state+'&u_card=${user.m_card}';
                                    ...
                                    <c:forEach var="o" items="${orders}">
                                    	<tr class="cart_item" id="ttr">
	                                        ...
	                                        <td class="product-thumbnail">
	                                        	...
	                                        	<a class="xq" onclick="funXq('${o.o_code }')" href="javascript:;" style="text-decoration:none;border-radius:5px; border:1px solid #99CCFF;background-color:#99CC99; font-family:楷体;font-size:1.3em;color:blue;font-weight: 900;">详情</a>
	                                        	<a href="#" onclick="dydd('${o.o_code }','${o.user.u_name}')" style="text-decoration:none;border-radius:5px; border:1px solid #99CCFF;background-color:#FFFFCC; font-family:楷体;font-size:1.3em;color:green;font-weight: 900;">下载合同</a>
	                                        	</c:if>
	                                        </td>
                                   	    </tr>
                                    </c:forEach>
...
function payOrder(o_state,o_code){
	if("未支付"==o_state){
		...
		if(b){
			window.location.href='${pageContext.request.contextPath}/order/pay?o_code='+o_code+'&u_card=${user.m_card}';
		}else{}
	}else{
...
function qxdd(o_code,o_state){
	if("未支付"==o_state){
		...
		if(b){
			window.location.href='${pageContext.request.contextPath}/order/qxOrder?o_code='+o_code+'&u_card=${user.m_card}';
		}else{}
	}else{
		alert("你的操作有误，请查看你的订单状态!");
	}
